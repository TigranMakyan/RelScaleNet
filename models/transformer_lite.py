...
import torch
tr = TransformerEncoderLite(d_model=512, dropout=0.1, nhead=8, dim_feedforward=2048)
input_list = [{'feat': torch.randn(size=(1024, 4, 256)),\
               'pos': torch.randn(size=(1024, 4, 256)),\
               'mask': torch.randn(size=(4, 512*512))},\
              {'feat': torch.randn(size=(1024, 4, 256)),\
               'pos': torch.randn(size=(1024, 4, 256)),\
               'mask': torch.randn(size=(4, 512*512))}]

q = torch.randn(size=(1024, 4, 512))
k = torch.randn(size=(2048, 4, 512))
v = torch.randn(size=(2048, 4, 512))
mask = torch.randn(size=(4, 2048))
