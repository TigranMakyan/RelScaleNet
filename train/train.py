...
from utils.utils import SaveBestModel, save_model, params_count_lite, create_cfg_lists, criterion_KL_divergence, _initialize_weights
...
parser.add_argument('-cfg', '--cfg', type=str, default='./first45.txt', help='names of folders')
...
print(model)
#checkpoint = torch.load('/home/powerx/computer_vision/RelScale/models/sh_bal_data_model_7_epoch.pth')
#model.load_state_dict(checkpoint['model_state_dict'])
params_count_lite(model)

optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE, weight_decay=0)
#optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[10, 15, 20], gamma=0.3)
...
for epoch in range(EPOCHS):
    ...
    save_model(epoch, model, optimizer, criterion, path_to_save=f'./models/sh_bal_data_model_{epoch}_epoch.pth')
    print('=' * 75)
